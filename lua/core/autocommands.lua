...
-- Handle NvimTree properly on quit or buffer delete
vim.api.nvim_create_autocmd({ "BufEnter", "QuitPre" }, {
  group = augroup("tree_quit_behavior"),
  callback = function(e)
    local ok, tree = pcall(require, "nvim-tree.api")
    if not ok or not tree.tree.is_visible() then
      return
    end
    ...
    if e.event == "QuitPre" and winCount == 2 then
      vim.cmd("qall")
    elseif e.event == "BufEnter" and winCount == 1 then
      vim.defer_fn(function()
        tree.tree.toggle({ find_file = true, focus = true })
        tree.tree.toggle({ find_file = true, focus = false })
      end, 10)
    end
  end,
})
